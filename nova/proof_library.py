...
FIXED in v0.2.0:
- Added content-based similarity matching (beyond just name matching)
- Added get_stats() method for introspection
- Added find_by_tag() for tag-based search
- Pattern suggestions now include token-count estimates
- Fixed missing __init__.py for tests
"""
...
class ProofLibrary:
    ...
    def find_by_tag(self, tag: str) -> list[ProofSkeleton]:
        """Find skeletons with a specific tag."""
        query = tag.lower()
        return [
            s for s in self._skeletons.values()
            if any(query in t.lower() for t in s.tags)
        ]
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "strategies": list(set(s.strategy for s in self._skeletons.values())),
            "verified_proofs": sum(1 for p in self._proofs.values() if p.verified),
            "total_lemmas_used": sum(len(p.lemmas_used) for p in self._proofs.values()),
        }

    def count_tokens_estimate(self, skeleton: ProofSkeleton) -> int:
        """Rough token count estimate for a proof skeleton."""
        return len(skeleton.theorem_template) // 4 + len(skeleton.proof_template) // 4
