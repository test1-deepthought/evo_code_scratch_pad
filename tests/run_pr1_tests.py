#!/usr/bin/env python3
"""
Standalone test runner for PR #1 - Shared KB Protocol (Pattern D).
Run: python tests/run_pr1_tests.py
"""
import os
import sys
...
def test_esc_handles_special_chars():
    r = _esc("it's a \"test\"\nwith newline")
    assert "\\'" in r, f"Should escape single quotes: {r}"
    assert "\\n" in r, f"Should escape newlines: {r}"
...
def test_full_critic_loop():
    ...
    try:
        ...
        mind.critique_strategy(claimed, gap="Missing explicit p=3 case",
                               severity="low",
...
def test_backtrack_flow():
    ...
    try:
        ...
        evo.write_trace("REASON", "conclusion(direct_factorisation_impossible)",
                        "failed", "No algebraic factorisation found")
        ...
        assert new_sid == "strat_backtrack_strat_direct", "Backtrack strategy name mismatch"
...
def test_retractall_removes_facts():
    ...
    try:
        ...
        assert "gap1" in old, "gap1 should be in returned old content"
        assert "gap2" in old, "gap2 should be in returned old content"
...
def test_empty_kb_query_does_not_crash():
    ...
    try:
        ...
        assert r is not None, "Query should return something"
    finally:
        cleanup(kb)
...
def test_unicode_in_facts():
    ...
    assert "²" in e or "\\\\" in e, "Unicode should be preserved or escaped"

test("unicode_in_facts", test_unicode_in_facts)

def test_long_fact_truncation():
    e = _esc("A" * 1000)
    assert "[...truncated]" in e, "Long strings should be truncated"
