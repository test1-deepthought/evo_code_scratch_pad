...
4. **Graceful degradation** — when a backend fails after all retries fall
   back to a degraded (cached — not implemented yet) or alternative result
   instead of crashing the main agent loop.
...
_CIRCUIT_STATE: dict[str, dict] = {}  # endpoint_key -> state dict
...
def _circuit_breaker(endpoint: str) -> CircuitBreakerState:
    ...
    # Fallback (no threading): simple dict access
    if endpoint not in _CIRCUIT_STATE:
        _CIRCUIT_STATE[endpoint] = CircuitBreakerState()
    return _CIRCUIT_STATE[endpoint]
...
_URLLIB_POOL: dict[str, Any] = {}  # base_url -> opener


def _get_opener_for_base(base_url: str, timeout: int = 15) -> Any:
    """Return a cached ``urllib.request.OpenerDirector`` with connection
    reuse and sensible timeouts for *base_url*.

    Uses HTTP keep-alive headers so that multiple requests to the same
    origin share a single TCP connection where possible.
    """
    ...
    # Build a custom opener that reuses connections via keep-alive
    opener = urllib.request.build_opener()
    # Add default headers that encourage keep-alive
    opener.addheaders = [
...
def resilient_call(
    ...
    **kwargs: Any,
    ...
    ----------
    operation:
        Callable to invoke (with *args, **kwargs).
    endpoint:
        Logical endpoint name for circuit breaker tracking.
    max_retries, base_delay, max_delay, backoff_multiplier:
        Standard retry parameters (passed to ``RetryWithBackoff.execute``).
    should_retry:
        Optional predicate ``(exc) -> bool``.  When ``None``, every exception
        is retried (up to *max_retries*).
    on_retry:
        Optional callback ``(exc, attempt, delay)``.
    circuit_breaker_threshold:
        If > 0, enable circuit breaker that opens after this many
        consecutive failures.
    circuit_breaker_recovery:
        Seconds before a HALF_OPEN probe is attempted.
    fallback:
        Optional callable that is invoked when all retries + circuit breaker
        reject the call.  Its return value becomes the result (caller should
        check for degraded semantics).
    """
    ...
    # --- Build default should_retry if none given ---
    ...
    # --- Wrap on_retry to update circuit breaker ---
    ...
    try:
        ...
        # All retries exhausted — record final failure on circuit breaker
        if cb is not None:
            cb.record_failure()
        if fallback is not None:
...
# ---------------------------------------------------------------------------
# Convenience: create a resilient "callable" for a function
# ---------------------------------------------------------------------------
...
def resilient(
    ...
    *,
    ...
    Usage::
        ...
        safe_get = resilient(requests.get, endpoint="my-api", max_retries=5)
        safe_get("https://example.com/data")
    """
...
def urlopen_retry(
    ...
    *,
    ...
    Returns the HTTP response object (``http.client.HTTPResponse``).
    ...
    def _is_retryable(exc: Exception) -> bool:
        if isinstance(exc, urllib.error.HTTPError):
            ...
            if exc.code in (429, 500, 502, 503, 504):
                return True
            return False
        # Network-level errors (URLError, ConnectionError, timeout) are retryable
...
def check_endpoint_health(
    ...
    """Quick health-check for an HTTP endpoint.

    Returns ``{"alive": bool, "latency_ms": float, "status": int}``.
    No retry — intended for fast checks only.
    """
    import urllib.request
    import urllib.error
...
# ---------------------------------------------------------------------------
# DNS caching (lightweight — prevents repeated resolution)
# ---------------------------------------------------------------------------

_DNS_CACHE: dict[str, str] = {}
_DNS_CACHE_TTL: float = 300.0  # 5 minutes


def _cached_resolve(hostname: str) -> str | None:
    """Resolve *hostname* to an IP, caching the result for ``_DNS_CACHE_TTL``.
    ...
    if cached is not None:
        # Simple TTL: store as "ip|timestamp"
        parts = cached.split("|")
    ...
    try:
        ...
        # Cache the failure briefly to avoid repeated DNS storms
...
#: Rate limiter for GitHub API (conservative — 10 req/s max)
